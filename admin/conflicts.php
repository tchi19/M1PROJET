...
// Allow Admin, Prof (Chefs only), and Doyen
require_role(['admin', 'prof', 'doyen']);

// Additional check for Profs: must be Chef de Departement
if (has_role('prof') && !has_role('admin') && !has_role('doyen')) {
...
// Logic for Professors (Chefs) - restrict to their department
if (has_role('prof') && !has_role('admin') && !has_role('doyen')) {
    $prof_data = get_professor_by_user_id($user['id']);
    // Filter by Dept
    $dept_sql = "SELECT id FROM departements d WHERE d.chef_id = ?";
    $dept_stmt = $conn->prepare($dept_sql);
    $dept_stmt->bind_param("i", $prof_data['id']);
    $dept_stmt->execute();
    ...
    $all_conflicts_sql .= " AND (c.department_id = $dept_id OR f.department_id = $dept_id)";
}
// Admins and Doyens see ALL conflicts (no extra filter)
...
<head>
    ...
    <title>Conflicts - Admin/Doyen Dashboard</title>
    <link rel="stylesheet" href="../css/style.css">
</head>
<body>
    <div class="navbar">
        <div class="navbar-brand">Exam Timetable Management - Administration</div>
        <div class="navbar-menu">
            <span><?php echo htmlspecialchars($user['full_name']); ?> 
            (<?php echo has_role('admin') ? 'Admin' : (has_role('doyen') ? 'Doyen' : 'Chef'); ?>)</span>
            <a href="../auth/logout.php">Logout</a>
        </div>
    </div>

    <div class="sidebar">
        <!-- Sidebar links might need adjustment for Doyen vs Admin, but assuming Doyen uses Admin layout for now -->
        ...
        <?php else: ?>
        <!-- Doyen Links - Simplified for conflict view task, keeping basic nav -->
        <a href="conflicts.php" class="active">Conflicts</a>
        <!-- Add other Doyen links if known, ensuring at least Dashboard and Logout work -->
        <?php endif; ?>
    </div>
